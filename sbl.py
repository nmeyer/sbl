...
class Goal():
    ...
    def output_state(self, indent_level=0):
        state = []
        if indent_level == 0:
            state.append('\t%s ( %s ):' % (self.label,' '.join([ dep.label for dep in self.deps.values() ])))
        for res in self.results.values():
            indent = "\t" * (indent_level + 2)
            state.append("%s-->%s" % (indent,res.label))
            state.append(res.output_state(indent_level + 1))
        state = filter(lambda s: len(s.strip()) > 0,state)
        return '\n'.join(state)
    
    def show_state(self, indent_level=0):
        print self.output_state(indent_level=indent_level)
...
class Interpreter():
    ...
    def output_state(self):
        """Output the current interpreter state"""
        ...
        for goal in self.goals.values():
            state.append(goal.output_state())
        state = filter(lambda s: len(s.strip()) > 0,state)
        return '\n'.join(state)
...
def main():
    ...
    open('interpreter_state.txt','w').write(interpreter.output_state())
